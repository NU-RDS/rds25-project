...
{
    float motor_angle = motorEncoder.readEncoderDeg();
    float sea_angle = SeaEncoder.readEncoderDeg();
...
void ForceControl::setMotorEncoder(int encoderCS)
{
    ...
    motorEncoder = new Encoder(encoderCS);
}

void ForceControl::setSeaEncoder(int encoderCS)
{
    if (SeaEncoder != nullptr) {
        delete SeaEncoder;  
    }
    SeaEncoder = new Encoder(encoderCS);
}